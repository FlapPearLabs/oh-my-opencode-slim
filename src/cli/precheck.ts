 ...
 *   - references_read assertions have referenceContent
...
function validateSuite(name: string, raw: string): Issue[] {
  ...
  suite.evals.forEach((evalCase, i) => {
    ...
    if (!rawCase || typeof rawCase.agent !== 'string' || rawCase.agent.trim() === '') {
    ...
    for (const assertion of evalCase.assertions) {
      if (assertion.type === 'references_read' && !assertion.referenceContent) {
        issues.push({
          ...ctx,
          message: `references_read assertion "${assertion.value}" is missing referenceContent`,
        });
      }
    }
  });
